...
async function main() {
  ...
  for (const modelName of args.models) {
    ...
    for (const { file, parsed } of targets) {
      ...
      if (!args.noCache && existsSync(cache)) {
        ...
        // unparseable graph is a real result worth keeping; a missing key,
        // a 429, or a dropped connection is not, and caching those would
        // silently poison every later run with a failure that never happened.
      ...
      // A warped image no longer matches the truth's coordinates, so position
      // error would be measuring the warp, not the model. Structural scoring
      // is unaffected.
  ...
  // ---- per model x tier ----
  console.log(
    `\n${"model".padEnd(20)} ${"tier".padEnd(13)} clean%  fixes  edgeF1  nodeF1  weight   $/img`,
  );
  const tsv = [
    ["model", "tier", "clean", "fixes", "edgeF1", "nodeF1", "weightAcc", "costUsd", "p50ms"].join("\t"),
  ];
  ...
  for (const modelName of args.models) {
    for (const tier of args.tiers) {
      ...
      const latencies = cell.map((r) => r.latencyMs).sort((a, b) => a - b);
      const v = {
        clean: mean(cell, (r) => (r.score.clean ? 1 : 0)),
        fixes: mean(cell, (r) => r.score.fixes),
        edgeF1: mean(cell, (r) => r.score.edges.f1),
        nodeF1: mean(cell, (r) => r.score.nodes.f1),
        weight: mean(cell, (r) => r.score.weightAccuracy),
        cost: mean(cell, (r) => r.costUsd),
  ...
  for (const tier of args.tiers) {
    const cell = rows.filter((r) => r.tier === tier);
    if (!cell.length) continue;
